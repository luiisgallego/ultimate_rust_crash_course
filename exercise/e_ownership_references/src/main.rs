...
fn change(content: &mut String) {
    if !content.ends_with('s') {
        content.push('s');
    }
}

fn eat(content: String) -> bool {
    content.starts_with('b') && content.contains('a')
}

fn bedazzle(content: &mut String) {
    *content = String::from("sparkly");
}
